 ...
 * @author Gihan
 */
public class StampDutyDidReport implements Printable {
    ...
    double grandstampDuty = 0;
    //-------------------------------
    int totalpages = 0;
    int currentpage = 0;

    public synchronized void printThisDetails(JTable jt, int month, int year) {
        this.jt = jt;
        this.month = month;
        this.year = year;
...
//        paper.setImageableArea(0, 0, 420, HEIGHT);
        ...
        for (int i = 0; i < jt.getRowCount(); i++) {
            ...
            grandEmploREM += Double.parseDouble(getvl(i, 19));
            grandstampDuty += getStampval(getvl(i, 20));
        }
...
//Company Name, Address & SlipHeading Details****************************************************************************        
        ...
        CentrePrint("STAMP DUTY DEDUCTION REPORT FOR " + getMonth(month).toUpperCase() + " " + year + "", WIDTH, -40, VariTopMargin += LINEHEIGHT + 2, g);
//Deaw a line to seperate------------------------------------------------------------------------------------------------
        ...
        if (currentpage == 1) {
            ...
            g.drawString("Stamp Duty Total : " + formatForDouble(grandstampDuty), LEFTMARGIN, VariTopMargin += LINEHEIGHT - 5);
        ...
        printColumn("Stamp", colwidth + 5, ((int) (colval += colwidth * 1.2)), 3);
        ...
        printColumn("Duty", colwidth + 5, ((int) (colval += colwidth * 1.2)), 3);
        ...
        double EmployeeStampDid = 0, remunation = 0;
        ...
        for (int i = startingIndex; i <= endindex; i++) {
            ...
            printColumn("", colwidth + 5, ((int) (colval += colwidth * 1.2)), 3);
            double rem = Double.parseDouble(getvl(i, 19));
            remunation += rem;
            printColumn(formatForDouble(rem), colwidth + 5, ((int) (colval += colwidth * 1.2)), 3);
            double empstampdid = getStampval(getvl(i, 20));
            EmployeeStampDid += empstampdid;
            printColumn(formatForDouble(empstampdid), colwidth + 5, ((int) (colval += colwidth * 1.2)), 3);
        ...
        printColumn(formatForDouble(EmployeeStampDid), colwidth + 5, ((int) (colval += colwidth * 1.2)), 3);
        ...
        if (totalpages == currentpage) {
            ...
            printColumn(formatForDouble(grandstampDuty), colwidth + 5, ((int) (colval += colwidth * 1.2)) + 20, 3);
            printColumn("", colwidth, ((int) (colval += colwidth * 1.2)), 3);
            colval = 0;
...
//            CentrePrint("Signature of Employer", WIDTH, -40, VariTopMargin += LINEHEIGHT, g);
    ...
    public double getStampval(String id){
    
        try {
            ResultSet sr = DB.getData("SELECT (FinalSalary-PackSal)AS stampdid FROM payroll WHERE payRollNum = '"+id+"'");
            if(sr.next()){
                id = sr.getString("stampdid");
            }else{
            id = "-1";
            }
    ...
    public static void main(String[] args) {
        StampDutyDidReport q = new StampDutyDidReport();
        synchronized (q) {
            q.printThisDetails(null, 0, 0);
        }
    }
}
