 ...
 * @author Imal
 */
public class MasterReport implements Printable {
    ...
    public synchronized int print(Graphics g, PageFormat pf, int pageIndex) throws PrinterException {
        ...
        try {
            ...
            g.setFont(fonth);
            CentrePrint("K.D.A. Weerasinghe & Co. (Pvt) Ltd", WIDTH, -500, VariTopMargin, g);
            g.setFont(fontn);
            CentrePrint("8/16, Thalapathpitiya Road, Nugegoda.", WIDTH, -500, VariTopMargin += LINEHEIGHT + 3, g);

            CentrePrint("MASTER REPORT ", WIDTH, -500, VariTopMargin += LINEHEIGHT + 3, g);

            VariTopMargin += LINEHEIGHT - 3;
            g.drawLine(LEFTMARGIN, VariTopMargin + 2, 660 + LEFTMARGIN, VariTopMargin + 2); //Draw line
            VariTopMargin += LINEHEIGHT - 3;
            g.setFont(fontd);
            columnVariable = 0;
            String print1 = "||" + createColumnStringcen1("Emp", column - 27) + "|"
                    + "|" + createColumnStringcen1("Full Name", (int) (column * 3)-30) + "|"
                    + "|" + createColumnStringcen1("Designation", (int) (column * 2) - 40) + "|"
                    + "|" + createColumnStringcen2("Basic ", column) + "|"
                    + "|" + createColumnStringcen2("Gov.", column - 10) + "|"
                    + "|" + createColumnStringcen2("Bud.", column - 7) + "|"
                    + "|" + createColumnStringcen2("Allow. >>", 2 * column - 27) + "|"
                    + "|" + createColumnStringcen2(" ", 2 * column - 37) + "|";
            columnVariable = 0;
            VariTopMargin += LINEHEIGHT * 1;
            String print12 = "||" + createColumnStringcen1("No", column - 27) + "|"
                    + "|" + createColumnStringcen1(" ", (int) (column * 3)-30) + "|"
                    + "|" + createColumnStringcen1("", (int) (column * 2) - 40) + "|"
                    + "|" + createColumnStringcen2("Salary", column) + "|"
                    + "|" + createColumnStringcen2("Allow.", column - 10) + "|"
                    + "|" + createColumnStringcen2("Allow.", column - 7) + "|";

            columnVariable = 0;
            VariTopMargin += LINEHEIGHT;
            VariTopMargin += 15;
            g.drawLine(LEFTMARGIN, VariTopMargin + 2, 660 + LEFTMARGIN, VariTopMargin + 2);
            VariTopMargin += LINEHEIGHT * 1;

            ResultSet rr = DB.getData("SELECT "
                    + " EmpNumber,EpfNumber,FMname,Lname,Designation,Basic_salary,gvtAllow,budAllow "
                    + "FROM allempdetails  ");
            ...
            while (rr.next()) {
                ...
                columnVariable = 0;
                String print2 = "||" + createColumnStringcen1(rr.getString("EmpNumber"), column - 27) + "|"
                        + "|" + createColumnStringcen1(rr.getString("FMname") + " " + rr.getString("Lname"), (int) (column * 2)) + "|"
                        + "|" + createColumnStringcen1(rr.getString("Designation"), (int) (column * 2) - 40) + "|"
                        + "|" + createColumnStringcen2(rr.getString("Basic_salary"), column) + "|"
                        + "|" + createColumnStringcen2(rr.getString("gvtAllow"), column - 10) + "|"
                        + "|" + createColumnStringcen2(rr.getString("budAllow"), column - 7) + "|"
                + "|" + createColumnStringcen2("     ", column - 7) + "|";

                ResultSet r1 = DB.getData("SELECT "
                        + " VALUE,NAME FROM "
                        + "     emp_has_extra  "
                        + "     LEFT JOIN extravalues  "
                        + "         ON (emp_has_extra.extraval_id = extravalues.extraval_id) WHERE TYPE='ALLOW' AND EmpNumber = '"+rr.getString("EmpNumber")+"'  ORDER BY EmpNumber");
                while(r1.next()){
                createColumnStringcen0(r1.getString("NAME"),r1.getString("VALUE"),2 * column - 27);
                }
                columnVariable = 0;
    ...
    public void createColumnStringcen0(String txt,String down, int len) {

        FontMetrics fm = g.getFontMetrics();
        int len1 = len - (fm.stringWidth(txt));
        int len2 = len - (fm.stringWidth(down));
        g.drawString(txt, LEFTMARGIN + columnVariable + len1+1, VariTopMargin + LINEHEIGHT);
        g.drawString(down, LEFTMARGIN + columnVariable + len2+1, VariTopMargin + LINEHEIGHT+LINEHEIGHT);
        columnVariable += len;
